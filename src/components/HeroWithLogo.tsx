...
export function HeroWithLogo({
  ...
  return (
    <section
      id="hero-section"
      className="relative overflow-hidden bg-[#192153] px-6 py-24 text-white sm:px-8 lg:px-10 lg:py-32"
    >
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col items-center text-center">
        {/* Logo Animation - Premium fade-in with translateY */}
        <motion.div
        ...
        >
          <h1 className="text-4xl font-black leading-tight tracking-[-0.03em] text-white sm:text-5xl md:text-6xl lg:text-7xl">
            ...
            <span className="text-[#B1D2FA]"> artificial intelligence</span>
          </h1>
        ...
        >
          {[
            ...
            <div
              key={item.label}
              className="rounded-[1.25rem] border border-white/10 bg-[#192153]/70 p-4"
            >
              <div className="text-3xl font-black text-white">{item.value}</div>
              <div className="mt-1 text-[11px] font-semibold uppercase tracking-[0.3em] text-[#B1D2FA]">
                {item.label}
              </div>
            </div>
          ))}
